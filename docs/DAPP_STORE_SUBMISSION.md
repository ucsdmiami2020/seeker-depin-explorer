...
Current build: **v1.4.1, versionCode 7** (wallet, guided tour and help centre, publisher Seeker ID, Play Solana PSG1).
...
- [ ] Verify the manifest: `aapt dump badging app-release.apk | grep -E "package|uses-permission|sdkVersion|targetSdkVersion"` — expect `versionCode='7'` (or higher), `targetSdkVersion:'36'`, and still only `INTERNET` + `VIBRATE` (MWA needs no extra permission).
